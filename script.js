document.addEventListener('DOMContentLoaded', () => {
    ...
    function generateMediaEmbed(link, fullSize = false) {
        ...
        if (youtubeId) {
            ...
            if (fullSize) {
                return `<div class="relative w-full max-w-4xl mx-auto mb-8"><div class="relative pb-[56.25%] h-0"><iframe class="absolute top-0 left-0 w-full h-full rounded-lg shadow-lg" src="${embedUrl}" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe></div></div>`;
            } else {
                return `<iframe width="100%" height="95" class="rounded-md" src="${embedUrl}" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>`;
            }
        }
    ...
    const navContentEl = document.getElementById('nav-content');
    const navFaqEl = document.getElementById('nav-faq');
    const toggleReadingModeBtnEl = document.getElementById('toggle-reading-mode');
    ...
    if (weekFilterEl) {
        weekFilterEl.addEventListener('change', (e) => {
    ...
    if (modalShareEl) modalShareEl.addEventListener('click', () => {
        copyToClipboard(window.location.href, '🔗 Enlace del boletín copiado al portapapeles');
    });

    if (navContentEl) navContentEl.addEventListener('click', () => {
        showTableOfContents();
    ...
    if (navFaqEl) navFaqEl.addEventListener('click', () => {
        showFAQ();
    ...
    if (toggleReadingModeBtnEl) toggleReadingModeBtnEl.addEventListener('click', () => {
        modalEl.classList.toggle('reading-mode');
    });
